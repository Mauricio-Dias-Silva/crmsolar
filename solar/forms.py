...
class ProjetoForm(forms.ModelForm):
    ...
    # 2. ADICIONE TODO ESTE MÉTODO NOVO AQUI DENTRO DA CLASSE
    def clean_valor_total(self):
        valor = self.cleaned_data.get('valor_total')
        if isinstance(valor, str):
            # Remove "R$", pontos e substitui a vírgula por ponto decimal
            valor_limpo = re.sub(r'[^\d,]', '', valor).replace(',', '.')
            try:
                # Converte para um número (Decimal ou float, dependendo do seu model)
                return float(valor_limpo) 
            except (ValueError, TypeError):
                raise forms.ValidationError("Por favor, insira um valor numérico válido.")
        return valor
